...
@ActiveProfiles("e2e")
class NeckRepositorySearchTest {
    @Autowired EntityManager em;
    @Autowired NeckRepository repository;
    private NeckService service;
    ...
    @BeforeEach void setup() {
        ...
        service = new NeckService(repository, null);
    }
    private Row add(String suffix, String state, LocalDateTime updated, LocalDateTime available) {
        long id = ((Number) em.createNativeQuery("INSERT INTO t_neck (serial_no, model_name, current_process, status, updated_at, available_at) "
    ...
    private Page<Neck> page(String category, int number) {
        return service.searchNecksPaged(category, prefix, "", "", "", number);
    }
    private List<Long> ids(Page<Neck> page) { return page.getContent().stream().map(Neck::getId).toList(); }
    ...
    @Test void literalSearchAndExactFiltersAndCategoryCounts() {
        ...
        assertEquals(List.of(match.id()), ids(service.searchNecksPaged("invalid", " " + prefix.toUpperCase(Locale.ROOT) + "%_! ",
                " model%_!i ", " proc ", " waiting ", 0)));
        for (String literal : List.of("%", "_", "!")) {
            assertEquals(List.of(match.id()), ids(service.searchNecksPaged("active", prefix + "%_!", literal, "", "", 0)));
        }
        assertEquals(0, service.searchNecksPaged("active", prefix, "", "Pr", "", 0).getTotalElements());
        assertEquals(0, service.searchNecksPaged("active", prefix, "", "", "WAIT", 0).getTotalElements());
        assertEquals(0, service.searchNecksPaged("passed", prefix, "", "", "WAITING", 0).getTotalElements());
    ...
    @Test void categoriesSortNullsAndPagingAreStable() {
        for (String category : List.of("active", "attention", "passed")) {
            ...
            var c = new NeckSearchCriteria(category, states, prefix.toLowerCase(Locale.ROOT), "", "", "");
    ...
    @Test void literalWildcardsDoNotMatchOtherCharacters() {
        for (String symbol : List.of("%", "_", "!")) {
            ...
            em.createNativeQuery("UPDATE t_neck SET model_name=:name WHERE id=:id")
                    .setParameter("name", "M" + symbol).setParameter("id", literal.id()).executeUpdate();
            em.createNativeQuery("UPDATE t_neck SET model_name='MZ' WHERE id=:id")
                    .setParameter("id", other.id()).executeUpdate();
            assertEquals(List.of(literal.id()), ids(service.searchNecksPaged("active", prefix + "x" + symbol, "", "", "", 0)));
            assertEquals(List.of(literal.id()), ids(service.searchNecksPaged("active", prefix, "M" + symbol, "", "", 0)));
        }
    }
    ...
    @Test void pageUsesOneCountAndDoesNotLoadRelatedOrder() {
        ...
        em.createNativeQuery("UPDATE t_neck SET production_order_id=:order WHERE id=:id")
    ...
    @Test void emptyAndSinglePagesAndNullFields() {
        ...
        em.createNativeQuery("UPDATE t_neck SET model_name=NULL, current_process=NULL WHERE id=:id")
        ...
        assertEquals(0, service.searchNecksPaged("active", prefix, "missing", "", "", 0).getTotalElements());
    }
}
