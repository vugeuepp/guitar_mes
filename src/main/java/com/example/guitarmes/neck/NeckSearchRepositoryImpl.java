...
public class NeckSearchRepositoryImpl implements NeckSearchRepository {
    private final EntityManager em;
    public NeckSearchRepositoryImpl(EntityManager em) { this.em = em; }

    @Override
    public Page<Neck> search(NeckSearchCriteria criteria, Pageable pageable) {
        ...
        var query = cb.createQuery(Neck.class);
        var root = query.from(Neck.class);
        ...
        var rows = em.createQuery(query).setHint("jakarta.persistence.fetchgraph", em.createEntityGraph(Neck.class))
    ...
    @Override
    public long countMatching(NeckSearchCriteria criteria) {
        var cb = em.getCriteriaBuilder();
    ...
    private Predicate[] predicates(CriteriaBuilder cb, Root<Neck> root, NeckSearchCriteria c) {
    ...
    private List<Order> sort(CriteriaBuilder cb, Root<Neck> root, String category) {
